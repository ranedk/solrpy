...
class SolrException(Exception):
    ...
    def __str__(self):
        return 'HTTP code=%s, reason=%s \n body=%s' % (self.httpcode, self.reason, self.body)
...
class SolrConnection:
    ...
    def set_query_params(self, q, fields=None, highlight=None,
        ...
        Sets parameters on which queries will be executed using get_batch
        returns an empty response object to make get_batch call
        """
    ...
    def query(self, q, fields=None, highlight=None,
        ...
        if sort:
            ...
            if not isinstance(sort, basestring):
                sort = ",".join(sort)
            """ If sort is of the form 'fieldname asc' or 'fieldname desc'
            then no need to add sort_order again. this happens when
            call to query (this function) is made from one of the internal
            functions after setting up the query params by the first 
    ...
    def raw_query(self, **params):
        ...
        p = {}
        for key, value in params.items():
            p[key.replace('_','.')] = value

        request = self.urlencode2(p)
    ...
    def _post(self, url, body, headers):
        ...
        while attempts:
            try:
                ...
                print url
                import pprint
                print "------ Http Call to Solr Server ----- "
...
class Response(object):
    ...
    def get_batch(self, rows=10, start=0):
        """
        Accepts start and rows to get any batch of results
